...
//        }));
    ...
    public void testAnalyzeWithSingleActionWithBoolCondition1() throws Exception {
        Smodel smodel = createSmodelWithSingleActionWithBoolCondition(true);
        SmodelInterpreter interpreter = new SmodelInterpreter(smodel, fvp);
    ...
    public void testAnalyzeWithSingleActionWithBoolCondition2() throws Exception {
        Smodel smodel = createSmodelWithSingleActionWithBoolCondition(false);
        SmodelInterpreter interpreter = new SmodelInterpreter(smodel, fvp);
    ...
    public void testAnalyzeWithSingleActionWithBoolCondition3() throws Exception {
        Smodel smodel = smodelFactory.createSmodel();
        Action expectedAction = createSimpleAction("expectedAction");
        smodel.getActions()
            .add(expectedAction);
        BoolLiteral boolLiteral = smodelFactory.createBoolLiteral();
        boolLiteral.setTrue(false);
        Expression boolExpr = smodelFactory.createExpression();
        boolExpr.setLiteral(boolLiteral);
        Expression expr = smodelFactory.createExpression();
        expr.setLeft(boolExpr);
    ...
    public void testPlanWithSingleActionWithBoolCondition1() throws Exception {
        Action expectedAction = createSimpleAction("expectedAction");
        Smodel smodel = createSmodelWithSingleActionWithBoolCondition(expectedAction, true);
        SmodelInterpreter interpreter = new SmodelInterpreter(smodel, fvp);
...
//    }
    ...
    private Action createSimpleAction(String name) {
        ...
        a.setName("testAction");
        ActionArguments actionArguments = smodelFactory.createActionArguments();
        a.setArguments(actionArguments);
        return a;
    }

    private Smodel createSmodelWithSingleActionWithBoolCondition(boolean value) {
        Action a = createSimpleAction("testAction");
        return createSmodelWithSingleActionWithBoolCondition(a, value);

    }

    private Smodel createSmodelWithSingleActionWithBoolCondition(Action a, boolean value) {
        Smodel smodel = smodelFactory.createSmodel();
        smodel.getActions()
            .add(a);
        BoolLiteral boolLiteral = smodelFactory.createBoolLiteral();
        boolLiteral.setTrue(value);
        Expression boolExpr = smodelFactory.createExpression();
        boolExpr.setLiteral(boolLiteral);
        Expression expr = smodelFactory.createExpression();
        expr.setLeft(boolExpr);
        IfStatement ifStmt = smodelFactory.createIfStatement();
        ifStmt.setCondition(expr);
        ActionCall thenStmt = smodelFactory.createActionCall();
        thenStmt.setActionRef(a);
