...
public class LoadBalancingSimulationExecutor extends PcmExperienceSimulationExecutor {
    ...
    protected void init() {
        ...
        initializeExperiment(simExpPcmConfiguration);

        this.pcmMeasurementSpecsBuilder = new LoadBalancingPcmMeasurementSpecificationBuilder();
        this.pcmMeasurementSpecsProvider = new PcmMeasurementSpecProvider(getExperiment(), pcmMeasurementSpecsBuilder);
        this.simExpSimulationConfiguration = new SimExpSimulationConfig(
                pcmMeasurementSpecsProvider.getPcmMeasurementSpecs());
        ...
        ProbabilityDistributionFactory.get()
            .register(new MultinomialDistributionSupplier());
    ...
    public void evaluate() {
        String reconfigurationPolicyId = simExpSimulationConfiguration.getReconfigurationPolicy()
            .getId();
        String sampleSpaceId = SimulatedExperienceConstants
            .constructSampleSpaceId(simExpSimulationConfiguration.getSimulationId(), reconfigurationPolicyId);
        SimulatedExperienceEvaluator evaluator = SimulatedExperienceEvaluator
            .of(simExpSimulationConfiguration.getSimulationId(), sampleSpaceId);
        LOGGER.info("***********************************************************************");
        LOGGER.info(String.format("The total Reward of policy %1s is %2s", reconfigurationPolicyId,
                evaluator.computeTotalReward()));
        LOGGER.info("***********************************************************************");
    }
    ...
    protected ExperienceSimulator createSimulator() {
        ...
        Resource usageModel = experiment.getInitialModel()
            .getUsageModel()
            .eResource();
