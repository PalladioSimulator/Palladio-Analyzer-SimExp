...
public class ReliabilityPrioritizedStrategy extends ReconfigurationStrategy<QVToReconfiguration> implements Initializable {
	...
	protected void monitor(State source, SharedKnowledge knowledge) {
		...
		var envState = sasState.getPerceivedEnvironmentalState().getValue().getValue();
		for (InputValue each : RobotCognitionEnvironmentalDynamics.toInputs(envState)) {
			...
			} else if(template.getId().equals(IMG_NOISE_TEMPLATE)) {
	...
	protected boolean analyse(State source, SharedKnowledge knowledge) {
		...
		var sensorNoise = knowledge.getValue(IMG_NOISE_TEMPLATE)
				...
				.get();
		var isSensorNoiseIncreased = !sensorNoise.get().equals("(SensorNoise=Low)");
