...
public class SystemConfigurationTracker {
	...
	private final static String SIMULATED_EXPERIENCE_BASE_FOLDER = Paths
			.get(ResourcesPlugin.getWorkspace().getRoot().getLocation().toString(), "resource", "DeltaIoT").toString();
	...
	private SystemConfigurationTracker(String strategyId) {
		this.strategyId = strategyId;
		this.configurations = Lists.newArrayList();
		this.run = 0;
	}
	
	public static SystemConfigurationTracker get(String strategyId) {
		if (instance == null) {
			instance = new SystemConfigurationTracker(strategyId);
		}
		return instance;
	}
	...
	private void registerHeader() {
		var header = new StringBuilder()
				.append("Run").append(CSV_DELIMITER)
				.append("Link").append(CSV_DELIMITER)
				.append("Power").append(CSV_DELIMITER)
				.append("Distribution")
				.toString();
	...
	private void register(WirelessLink link) {
		String configuration = new StringBuilder()
				.append(run).append(CSV_DELIMITER)
				.append(link.pcmLink.getEntityName()).append(CSV_DELIMITER)
				.append(link.transmissionPower).append(CSV_DELIMITER)
	...
	private void print(WirelessLink eachLink) {
		System.out.println(String.format("Link: %1s, Power: %2s, SNR:  %3s, Dist.: %4s", eachLink.pcmLink.getEntityName(),
	...
	private String getFileLocation(String strategyId) {
		...
		return Paths.get(SIMULATED_EXPERIENCE_BASE_FOLDER, csvFileName).toString();
	}
