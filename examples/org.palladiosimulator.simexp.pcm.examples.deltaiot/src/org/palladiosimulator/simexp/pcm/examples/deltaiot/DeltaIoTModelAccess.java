...
public class DeltaIoTModelAccess {
	...
	public static DeltaIoTModelAccess get() {
		return INSTANCE;
	}

	public double retrieveWirelessInterference(GroundRandomVariable maVariable,
			PerceivableEnvironmentalState nextEnvironment) {
		return toInputs(nextEnvironment.getValue().getValue()).stream()
				.filter(v -> v.variable.getId().equals(maVariable.getId())).map(InputValue::asCategorical)
				.map(v -> Double.valueOf(v.get())).findFirst()
	...
	public int retrieveTransmissionPower(AssemblyContext mote, LinkingResource link) {
		...
		if (varUsages.size() == 1) {
			pcmVar = varUsages.get(0).getVariableCharacterisation_VariableUsage().get(0)
	...
	public AssemblyContext findSourceMote(LinkingResource link, ArchitecturalConfiguration<?> archConfig) {
		PCMInstance pcm = PcmArchitecturalConfiguration.class.cast(archConfig).getConfiguration();
	...
	public boolean isTransmissionPowerOfLink(VariableUsage varUsage, LinkingResource link) {
		String refNameSuffix = varUsage.getNamedReference__VariableUsage().getReferenceName()
				.replaceFirst("TransmissionPower", "");
		String linkNameSuffix = link.getEntityName().replaceFirst("Unicast", "");
	...
	private PCMRandomVariable retrieveTransmissionPower(List<VariableUsage> varUsages, LinkingResource link) {
		for (VariableUsage each : varUsages) {
			if (isTransmissionPowerOfLink(each, link)) {
				return each.getVariableCharacterisation_VariableUsage().get(0)
						.getSpecification_VariableCharacterisation();
			}
		}
	...
	private AssemblyContext resolveSourceMote(List<AllocationContext> allocContexts, System system) {
		...
		AssemblyContext first = allocContexts.get(0).getAssemblyContext_AllocationContext();
		AssemblyContext second = allocContexts.get(1).getAssemblyContext_AllocationContext();
	...
	private boolean areIndirectlyConnected(AssemblyContext source, AssemblyContext target, System system) {
		...
		for (AssemblyContext each : filterAssemblyContextsWithSource(transmitter, system)) {
			if (each.getId().equals(target.getId())) {
				return true;
			}
		}
		return false;
	...
	private List<AssemblyContext> filterAssemblyContextsWithSource(AssemblyContext source, System system) {
		return system.getConnectors__ComposedStructure().stream().filter(AssemblyConnector.class::isInstance)
				...
				.filter(c -> c.getRequiringAssemblyContext_AssemblyConnector().getId().equals(source.getId()))
				.map(c -> c.getProvidingAssemblyContext_AssemblyConnector()).collect(toList());
	}
	...
	private boolean isMoteOrGatewayAllocation(AllocationContext allocContext) {
		String entity = allocContext.getAssemblyContext_AllocationContext().getEncapsulatedComponent__AssemblyContext()
				.getEntityName();
	...
	private Optional<ResourceContainer> findContainerFor(AllocationContext allocContext,
		...
		return container.stream()
				.filter(c -> c.getId().equals(allocContext.getResourceContainer_AllocationContext().getId()))
				.findFirst();
	}
	...
	private Function<ServiceEffectSpecification, Stream<ProbabilisticBranchTransition>> filterProbabilistBranches() {
		return seff -> {
			...
			TreeIterator<EObject> iterator = ResourceDemandingSEFF.class.cast(seff).eAllContents();
