...
public class PerformabilityRewardEvaluation implements RewardEvaluator {
    ...
    public Reward<Double> evaluate(StateQuantity quantity) {
        ...
        double lower = lowerResponseTimeThreshold.getSecond().getValue();
        double upper = upperResponseTimeThreshold.getSecond().getValue();
        ...
        Optional<SimulatedMeasurement> simulatedSuccessRateTimeMeasurement = quantity.findMeasurementWith(systemResultExectutionTypeTimeMeasurementSpec);
        ...
        if (!simulatedSuccessRateTimeMeasurement.isPresent()) {
            LOGGER.error(String.format("No simulated success rate measurements available for quantified state: %s; check your measurement specification."
                    , quantity.getMeasurements().toString()));
        } else {
            LOGGER.info(String.format("Simulated success rate measurements available for quantified state %s", quantity.toString()));
            measuredSuccessRate = simulatedSuccessRateTimeMeasurement.get().getValue();
        }
        
        PerformabilityRewardSignal rewardSignal = PerformabilityRewardSignal.create(measuredResponseTime, measuredSuccessRate, lower, upper);
