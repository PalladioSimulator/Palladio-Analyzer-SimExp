...
public class KmodelValidator extends AbstractKmodelValidator {
	...
	public void checkKmodel(Kmodel model) {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			Collection<EStructuralFeature.Setting> fieldReferences = EcoreUtil.UsageCrossReferencer.find(field, field.eResource());
		...
		for (int i = 0; i < actions.size(); i++) {
			...
			Collection<EStructuralFeature.Setting> actionReferences = EcoreUtil.UsageCrossReferencer.find(action, action.eResource());
			
			if (actionReferences.isEmpty()) {
				warning("The action '" + action.getName() + "' is never used.", KmodelPackage.Literals.KMODEL__ACTIONS, i);
			}
		}
	...
	public void checkProbe(Probe probe) {
		...
		for (int i = 0; i < probes.size(); i++) {
			...
			if (probe.getId().equals(otherProbe.getId())) {
	...
	public void checkActionCall(ActionCall actionCall) {
		...
		if (action != null) {
			...
			for (int i = 0; i < arguments.size(); i++) {
				...
				if (!argument.getParamRef().equals(parameter)) {
	...
	// Returns the next expression in the tree that contains either an operation, a field reference or a literal.
	public Expression getNextExpressionWithContent(Expression expression) {
		if (expression.getOp() != Operation.UNDEFINED || expression.getFieldRef() != null || expression.getLiteral() != null) {
			return expression;
